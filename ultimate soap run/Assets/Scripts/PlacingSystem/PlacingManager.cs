...
public class BuildingManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        // Automatically populate meshComponents array
        if (meshComponents == null || meshComponents.Length == 0)
        {
            meshComponents = GetComponentsInChildren<MeshRenderer>();
        }
    ...
    public void SetMaterial(PlacementMode mode)
    {
        if (mode == PlacementMode.Fixed)
        {
            foreach (MeshRenderer r in meshComponents)
                r.materials = initialMaterials[r].ToArray();
        }
        else
        {
    ...
    {
        return ((BuildingPlacer.instance.groundLayerMask.value & (1 << o.layer)) != 0);
    }
